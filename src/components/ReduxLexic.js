...
const ReduxLexic = () => {
  return (
    <div className="lexic">
      <h2>Reflections on Redux</h2>
      <div>
        <ol>
          <li>The entire application doesn't connect to the redux-store. Only components <code>(containers)</code> with the Purpose of interracting with the store (fetch or dispatch) do so.</li>
        </ol>
      </div>
      ...
      <div>
        ...
        <ul>
          <li>Different parts of the code can’t change the state arbitrarily.</li>
      ...
      <div>
        ...
        <ol>
          <li>You call <code>store.dispatch(action)</code> from anywhere in your app, including components and XHR callbacks, or even at scheduled intervals.</li>
          ...
          <li>The root reducer may combine the output of multiple reducers into a single state tree.
          How you structure the root reducer is completely up to you. Redux ships with a 
          <GotoLink url="https://redux.js.org/api/combinereducers" opensNewTab>combineReducers()</GotoLink> helper function, 
          useful for “splitting” the root reducer into separate functions that each manage one branch of the state tree.


          </li>
      
          <li>The Redux store saves the complete state tree returned by the root reducer. This new tree is now the next state of your app! Every listener registered with store.subscribe(listener) will now be invoked; listeners may call store.getState() to get the current state.
            Now, the UI can be updated to reflect the new state. If you use bindings like <GotoLink url="https://github.com/gaearon/react-redux" opensNewTab>React Redux</GotoLink>, this is the point at which component.setState(newState) is called.</li>
        </ol>
      </div>
      <div>
      ...
      <div>
        ...
        <p>You can define a special function <code>mapStateToProps</code> which describes how to transform the current Redux store state into the props you want to pass to a presentational component.</p>
        <h3>Dispatching to Redux store</h3>
        <p>You can define a special function <code>mapDispatchToProps</code> which receives a <code>dispatch()</code> method and returns callback props that you want to inject into the presentational component.</p>
      </div>
      <div>
        <h2>Examples</h2>
        <ul>
...
const GotoLink = ({ url, icon, opensNewTab, children }) => {
  return (
    <a href={ url } target={ opensNewTab ? '_blank' : '_self' }>
      { icon && <IconLink /> }
      { children }
    </a>
  )
